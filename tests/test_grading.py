...
from easysnec.utils.grading import InputData, Grade, Course, OutputData, SuccessStatus, ScoreType, typed_function
import uuid
import pytest
...
def test_get_closest_course():
    ...
    assert generate_input_from_station_list([39, 31, 32, 35, 37]).get_closest_course(COURSES) == crab_course
    assert generate_input_from_station_list([39, 31, 32, 35, 36]).get_closest_course(COURSES) == crab_course
    assert generate_input_from_station_list([39, 31, 2001, 35, 37]).get_closest_course(COURSES) == crab_course
    assert generate_input_from_station_list([39, 32, 35, 37]).get_closest_course(COURSES) == crab_course
    assert generate_input_from_station_list([39, 31, 32, 35, 37]).get_closest_course(COURSES) == crab_course
    assert generate_input_from_station_list([38, 31, 32, 36, 37]).get_closest_course(COURSES) == crab_course
    assert generate_input_from_station_list([40, 36, 34, 31, 32, 33, 35, 37, 38, 39]).get_closest_course(COURSES) != crab_course
